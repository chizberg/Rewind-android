...
/**
 * Runs a test giving the reducer a scope on the test scheduler (NOT `backgroundScope`, whose
 * tasks `advanceUntilIdle()` ignores). The scope is cancelled afterwards so long-lived
...
class ReducerSyncEffectTest {
    @Test
    fun syncEffectRunsImmediatelyAfterReduce() =
        reducerTest { scope ->
            val reducer = makeReducer(scope)
            var ran = false
    ...
    @Test
    fun syncEffectsRunInEnqueueOrder() =
        reducerTest { scope ->
            val reducer = makeReducer(scope)
    ...
    @Test
    fun syncEffectMaySafelyDispatchFollowUpAction() =
        reducerTest { scope ->
            ...
            // no coroutine hop) and does not trip the recursion check.
            val reducer = makeReducer(scope)
    ...
    @Test
    fun syncEffectsRunBeforeAsyncEffectsAreScheduled() =
        reducerTest { scope ->
            // Headline invariant of the dual-effect model: within ONE reduce call,
...
class ReducerAsyncEffectTest {
    @Test
    fun multipleEffectsPerActionAllRun() =
        reducerTest { scope ->
            val reducer = makeReducer(scope)
            reducer(
    ...
    @Test
    fun performEffectDispatchesFollowUp() =
        reducerTest { scope ->
            val reducer = makeReducer(scope)
            reducer(
                TestAction.AsyncEffects(
    ...
    @Test
    fun anotherActionEffectDispatchesFollowUp() =
        reducerTest { scope ->
            val reducer = makeReducer(scope)
            reducer(
                TestAction.AsyncEffects(
    ...
    @Test
    fun afterEffectFiresAfterDelay() =
        reducerTest { scope ->
            val reducer = makeReducer(scope)
            reducer(
                TestAction.AsyncEffects(
...
class ReducerCancellationTest {
    @Test
    fun sameIdReplacesPendingEffect() =
        reducerTest { scope ->
            val reducer = makeReducer(scope)
                ...
                TestAction.AsyncEffects(
                    listOf(
                        AsyncEffect.after(
                            50.milliseconds,
                            id = "x",
                            anotherAction = TestAction.Add(1),
                        ),
                    ),
                ),
            )
    ...
    @Test
    fun cancelEffectStopsPendingFollowUp() =
        reducerTest { scope ->
            val reducer = makeReducer(scope)
            reducer(
                TestAction.AsyncEffects(
    ...
    @Test
    fun differentIdsRunIndependently() =
        reducerTest { scope ->
            val reducer = makeReducer(scope)
            reducer(
                TestAction.AsyncEffects(
    ...
    @Test
    fun effectIdCanBeReusedAfterCancel() =
        reducerTest { scope ->
            val reducer = makeReducer(scope)
            reducer(
                TestAction.AsyncEffects(
...
class ReducerDebounceTest {
    @Test
    fun debouncedCollapsesRapidDispatches() =
        reducerTest { scope ->
            val reducer = makeReducer(scope)
    ...
    @Test
    fun debouncedFiresExactlyAtDelayBoundary() =
        reducerTest { scope ->
            val reducer = makeReducer(scope)
            reducer(
                TestAction.AsyncEffects(
    ...
    @Test
    fun debouncedClosureFormRunsAfterDelay() =
        reducerTest { scope ->
            val reducer = makeReducer(scope)
            reducer(
                TestAction.AsyncEffects(
    ...
    @Test
    fun cancelDebouncedStopsPendingFollowUp() =
        reducerTest { scope ->
            val reducer = makeReducer(scope)
            reducer(
                TestAction.AsyncEffects(
            ...
            reducer(
                TestAction.AsyncEffects(
                    listOf(AsyncEffect.cancel(DebouncedActionId.RegionChanged)),
                ),
            )
...
class ReducerRecursionGuardTest {
    @Test
    fun recursiveDispatchDuringReduceThrows() =
        reducerTest { scope ->
            lateinit var reducer: Reducer<TestState, TestAction>
            reducer =
                Reducer(TestState(), scope) { state, _, _, _ ->
...
class ReducerExtensionTest {
    @Test
    fun onStateUpdateReceivesCurrentAndNewValues() =
        reducerTest { scope ->
            // Divergence from iOS: VGSL is synchronous; a StateFlow collector runs on `scope`.
            // Advancing the scheduler after each dispatch prevents conflation; all values observed.
    ...
    @Test
    fun addingFlowDispatchesAction() =
        reducerTest { scope ->
...
class ViewStoreTest {
    ...
    @Test
    fun dispatchesActionsAndReflectsState() =
        reducerTest { scope ->
